...
echo -e "${YELLOW}****************************${NC}"
echo -e "${YELLOW} MIGRATION ${OUTPUT_DIR} > PG   ${NC}"
echo -e "${YELLOW}****************************${NC}"
...
# SECONDS=0
# SQL="./output/${OUTPUT_DIR}/psql_tables.sql"
...
SECONDS=0
SQL="./output/${OUTPUT_DIR}/psql_data.sql"
echo -en "${YELLOW}Inserting data${NC}... add sslmode=require inside -d "
psql --set  ON_ERROR_STOP=on -h ${HOST} -U ${USER} -p ${PORT} -f ${SQL} -o ${ERRLOG} -d "dbname=${DB_NAME}" || error "Inserting data"

echo "Done ($SECONDS)"

# SECONDS=0
# SQL="./output/${OUTPUT_DIR}/psql_views.sql"
# if [ ${SKIP_VIEWS} = "false" ]
# then
...
# SECONDS=0
# SQL="./output/${OUTPUT_DIR}/psql_index_fk.sql"
